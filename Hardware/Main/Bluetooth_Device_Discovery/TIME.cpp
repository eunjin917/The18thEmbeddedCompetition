...
String now(){//현재시간을 YYYY-MM-DD 12:00:00 형식으로 출력
  t = rtc.getTime();
...
void time_setting(String t, String d){//아두이노에서 시간을 받아와서 DS1302에 시간 설정
  /*__TIME__ -> t "Feb 24 2011"형식으로 출력
    __DATE__ -> d "19:20:28"형식으로 출력 */
  rtc.halt(false); // 동작 모드로 설정
  ...
  int month;
  if(str1.compareTo("Jan")==1){
    month=1;
  }
  else if(str1.compareTo("Feb")==1){
    month=2;
  }
  else if(str1.compareTo("Mar")==1){
    month=3;
  }
  else if(str1.compareTo("Apr")==1){
    month=4;
  }
  else if(str1.compareTo("May")==1){
    month=5;
  }
  else if(str1.compareTo("Jun")==1){
    month=6;
  }
  else if(str1.compareTo("Jul")==1){
    month=7;
  }
  else if(str1.compareTo("Aug")==1){
    month=8;
  }
  else if(str1.compareTo("Sep")==1){
    month=9;
  }
  else if(str1.compareTo("Oct")==1){
    month=10;
  }
  else if(str1.compareTo("Nov")==1){
    month=11;
  }
  else if(str1.compareTo("Dec")==1){
    month=12;
  }
  
  rtc.setDate(str2.toInt(), month, str3.toInt()); // YYYY-MM-DD로 설정
}
